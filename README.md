...
3. 핵심 기능 및 기술적 고려 사항
    3.1. 다중 계층 유효성 검증 (Multi-layer Validation)
        ...
        확장자 입력 및 일괄 처리:
            사용자 편의성을 위해 확장자 입력 시 Tab 키를 누르면 임시 목록에 확장자가 추가되며, '추가' 버튼 클릭 시 임시 목록에 있는 모든 확장자를 한 번에 서버로 전송하여 저장합니다. 이 과정에서 클라이언트와 서버 양단에서 유효성 검증이 이루어집니다.

        서버측 중복 및 유효성 검증 최적화:
            `ExtensionService.addCustomExtensions` 메서드에서는 확장자 목록을 일괄 처리합니다. 이 과정에서 다음 순서로 중복 및 유효성 검사를 수행하여 데이터 무결성을 보장하고 불필요한 DB 접근을 최소화합니다.
            1.  요청 내 중복 확인: 현재 요청 내에서 동일한 확장자 이름이 여러 번 포함된 경우, 첫 번째를 제외한 모든 중복 항목은 실패로 처리합니다. (예: `["exe", "exe"]` 요청 시 두 번째 "exe"는 실패 처리)
            2.  전체 개수 제한 확인: 전체 커스텀 확장자 개수(최대 200개)를 초과하는 경우 해당 확장자는 실패로 처리합니다.
            3.  이름 형식 검증: 확장자 이름이 비어 있거나 1자 미만, 20자 초과인 경우 실패로 처리합니다. (클라이언트 단에서도 1차 검증)
            4.  고정 확장자 중복 확인: 시스템에 정의된 고정 확장자와 이름이 중복되는 경우 실패로 처리합니다.
            5.  기존 커스텀 확장자 중복 확인: 데이터베이스에 이미 존재하는 커스텀 확장자와 이름이 중복되는 경우 실패로 처리합니다.
            각 검증 단계에서 실패한 확장자는 상세한 실패 사유와 함께 클라이언트에 반환되는 `CustomExtensionBatchResultDto`에 포함됩니다.
